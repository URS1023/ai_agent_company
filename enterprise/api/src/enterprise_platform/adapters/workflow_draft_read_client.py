"""Bounded metadata-only native draft read; never retrieve or rewrite snapshots."""
...
from enterprise_platform.application.workflow_draft_read import (
    DraftReadRejected,
    NativeDraftMetadata,
    canonical_native_uuid,
)
...
class _Command(BaseModel):
    ...
    @field_validator("workspace_id", "app_id")
    @classmethod
...
class DifyWorkflowDraftReadClient:
    _base_url: str
    ...
    async def _body(self, response: httpx.Response) -> bytes:
        ...
        async for chunk in response.aiter_bytes():
            ...
            if size > self._max_response_bytes:
                raise ValueError("native_draft_read_response_too_large")
            chunks.append(chunk)
        return b"".join(chunks)

    async def read(self, principal: Principal, session: NativeSetupSession, *, app_id: str) -> NativeDraftMetadata:
        try:
            command = _Command(workspace_id=principal.workspace_id, app_id=app_id)
                ...
                async with httpx.AsyncClient(
                    ...
                    async with client.stream(
                        ...
                        headers={
                            ...
                            "X-Enterprise-Draft-Operation": "read-assessment-draft",
                        },
                    ) as response:
                        if (
                        ...
                        if result.app_id != command.app_id:
                            raise DraftReadRejected()
                        return NativeDraftMetadata(
                            workspace_id=command.workspace_id,
                            app_id=result.app_id,
                            draft_id=result.draft_id,
                            draft_hash=result.hash,
                        )
        except DraftReadRejected:
            raise
        except (httpx.HTTPError, TimeoutError, ValueError, EnterpriseError):
            raise DraftReadRejected() from None
