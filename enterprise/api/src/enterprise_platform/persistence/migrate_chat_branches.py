"""Explicit additive 0013 after verified 0001-0012; never migrates at startup."""
...
from . import migrate_chat_messages as previous
...
def require_branch_prerequisites(inspector: Inspector, *, schema: str = "public") -> None:
    require_schema(inspector, prerequisite_metadata(), schema=schema)


def chat_branches_statements() -> tuple[str, ...]:
    metadata = MetaData()
...
def render_chat_branches_sql() -> str:
    return (
        ...
        "BEGIN;\nSET LOCAL search_path TO public;\n\n" + ";\n\n".join(chat_branches_statements()) + ";\n\nCOMMIT;\n"
    )


def read_chat_branches_sql() -> str:
    artifact = (
        files("enterprise_platform.persistence")
        .joinpath("migrations/0013_chat_branches.sql")
        .read_text(encoding="utf-8")
    )
    if artifact != render_chat_branches_sql():
        raise InvalidInput("migration_artifact_mismatch")
    return artifact


def apply_chat_branches(database_url: str, expected_database: str) -> None:
    ...
    for read_artifact in (
        ...
        read_sql_drafts_sql,
        read_sql_trials_sql,
        previous.read_chat_messages_sql,
    ...
    try:
        with engine.begin() as connection:
            ...
            require_branch_prerequisites(inspect(connection))
            for statement in chat_branches_statements():
                connection.exec_driver_sql(statement)
    except SQLAlchemyError:
        raise PersistenceError("enterprise_branch_migration_failed") from None
    finally:
        engine.dispose()
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = _Parser(description="Explicit enterprise migration 0013 after verified 0001-0012")
    ...
    try:
        ...
        if args.print_sql:
            sys.stdout.write(read_chat_branches_sql())
            return 0
        ...
        apply_chat_branches(url, args.expected_database)
    except EnterpriseError as error:
        sys.stderr.write(error.code + "\n")
        return 1
    sys.stdout.write("Applied enterprise migration 0013.\n")
    return 0
