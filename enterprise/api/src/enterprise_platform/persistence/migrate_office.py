"""Explicit additive 0014 after verified 0001-0013; never migrates at startup."""
...
from . import migrate_chat_branches as previous
...
from .workbench_branches import BranchContextBase


def prerequisite_metadata() -> MetaData:
    metadata = previous.prerequisite_metadata()
    for table in BranchContextBase.metadata.sorted_tables:
        table.to_metadata(metadata)
    return metadata


def require_office_prerequisites(inspector: Inspector, *, schema: str = "public") -> None:
    require_schema(inspector, prerequisite_metadata(), schema=schema)


def office_statements() -> tuple[str, ...]:
    metadata = MetaData()
...
def render_office_sql() -> str:
    sql = (
        ...
        "BEGIN;\nSET LOCAL search_path TO public;\n\n" + ";\n\n".join(office_statements()) + ";\n\nCOMMIT;\n"
    )
    return "\n".join(line.rstrip() for line in sql.split("\n"))


def read_office_sql() -> str:
    artifact = (
        files("enterprise_platform.persistence").joinpath("migrations/0014_office.sql").read_text(encoding="utf-8")
    )
    if artifact != render_office_sql():
        raise InvalidInput("migration_artifact_mismatch")
    return artifact


def apply_office(database_url: str, expected_database: str) -> None:
    ...
    for read_artifact in (
        ...
        previous.read_chat_branches_sql,
        read_office_sql,
    ):
        read_artifact()
    ...
    try:
        with engine.begin() as connection:
            ...
            require_office_prerequisites(inspect(connection))
            for statement in office_statements():
                connection.exec_driver_sql(statement)
    except SQLAlchemyError:
        raise PersistenceError("enterprise_office_migration_failed") from None
    finally:
        engine.dispose()
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = _Parser(description="Explicit enterprise migration 0014 after verified 0001-0013")
    ...
    try:
        ...
        if args.print_sql:
            sys.stdout.write(read_office_sql())
            return 0
        ...
        apply_office(url, args.expected_database)
    except EnterpriseError as error:
        sys.stderr.write(error.code + "\n")
        return 1
    sys.stdout.write("Applied enterprise migration 0014.\n")
    return 0
