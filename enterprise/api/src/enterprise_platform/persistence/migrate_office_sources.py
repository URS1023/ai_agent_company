"""Explicit additive 0015 after verified 0001-0014; never migrates at startup."""
...
from . import migrate_office as previous
from .migrate import read_initial_sql, validate_target
from .migrate_chat_branches import read_chat_branches_sql
from .migrate_chat_messages import read_chat_messages_sql
...
from .migrate_sql_trials import read_sql_trials_sql
...
from .office_models import OfficeBase
from .office_source_models import OfficeSourceBase
...
def require_office_sources_prerequisites(inspector: Inspector, *, schema: str = "public") -> None:
    require_schema(inspector, prerequisite_metadata(), schema=schema)


def office_sources_statements() -> tuple[str, ...]:
    metadata = MetaData()
    for table in OfficeSourceBase.metadata.sorted_tables:
...
def render_office_sources_sql() -> str:
    sql = (
        ...
        "BEGIN;\nSET LOCAL search_path TO public;\n\n" + ";\n\n".join(office_sources_statements()) + ";\n\nCOMMIT;\n"
    )
    return "\n".join(line.rstrip() for line in sql.split("\n"))


def read_office_sources_sql() -> str:
    artifact = (
        files("enterprise_platform.persistence").joinpath("migrations/0015_office_sources.sql").read_text(encoding="utf-8")
    )
    if artifact != render_office_sources_sql():
        raise InvalidInput("migration_artifact_mismatch")
    return artifact


def apply_office_sources(database_url: str, expected_database: str) -> None:
    ...
    for read_artifact in (
        ...
        previous.read_office_sql,
        read_office_sources_sql,
    ):
        read_artifact()
    ...
    try:
        with engine.begin() as connection:
            ...
            require_office_sources_prerequisites(inspect(connection))
            for statement in office_sources_statements():
                connection.exec_driver_sql(statement)
    except SQLAlchemyError:
        raise PersistenceError("enterprise_office_sources_migration_failed") from None
    finally:
        engine.dispose()
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = _Parser(description="Explicit enterprise migration 0015 after verified 0001-0014")
    ...
    try:
        ...
        if args.print_sql:
            sys.stdout.write(read_office_sources_sql())
            return 0
        ...
        apply_office_sources(url, args.expected_database)
    except EnterpriseError as error:
        sys.stderr.write(error.code + "\n")
        return 1
    sys.stdout.write("Applied enterprise migration 0015.\n")
    return 0
