"""Explicit guarded additive 0008. No native app changes or automatic migrations."""
...
def require_schedule_prerequisites(inspector: Inspector, *, schema: str = "public") -> None:
    require_schema(inspector, prerequisite_metadata(), schema=schema)


def schedules_statements() -> tuple[str, ...]:
    metadata = MetaData()
...
def render_schedules_sql() -> str:
    return (
        "-- Enterprise migration 0008: single-owner schedules and revision-fenced lifecycle.\n"
        "-- Apply only after verified 0001, 0002, 0003, 0004, 0005, 0006 and 0007 "
        "in the dedicated enterprise_* database.\n\n"
        "BEGIN;\nSET LOCAL search_path TO public;\n\n" + ";\n\n".join(schedules_statements()) + ";\n\nCOMMIT;\n"
    )


def read_schedules_sql() -> str:
    artifact = (
        files("enterprise_platform.persistence").joinpath("migrations/0008_schedules.sql").read_text(encoding="utf-8")
    )
    if artifact != render_schedules_sql():
        raise InvalidInput("migration_artifact_mismatch")
    return artifact


def apply_schedules(database_url: str, expected_database: str) -> None:
    ...
    try:
        with engine.begin() as connection:
            ...
            require_schedule_prerequisites(inspect(connection))
            for statement in schedules_statements():
                connection.exec_driver_sql(statement)
    except SQLAlchemyError:
        raise PersistenceError("enterprise_schedules_migration_failed") from None
    finally:
        engine.dispose()
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = _Parser(
        description="Explicit enterprise migration 0008 after verified 0001, 0002, 0003, 0004, 0005, 0006 and 0007"
    )
    ...
    try:
        ...
        if args.print_sql:
            sys.stdout.write(read_schedules_sql())
            return 0
        ...
        apply_schedules(url, args.expected_database)
    except EnterpriseError as error:
        sys.stderr.write(error.code + "\n")
        return 1
    sys.stdout.write("Applied enterprise migration 0008.\n")
    return 0
