"""Explicit guarded additive 0011. No native app changes or automatic migrations."""
...
def require_sql_trial_prerequisites(inspector: Inspector, *, schema: str = "public") -> None:
    require_schema(inspector, prerequisite_metadata(), schema=schema)


def sql_trials_statements() -> tuple[str, ...]:
    metadata = MetaData()
...
def render_sql_trials_sql() -> str:
    return (
        "-- Enterprise migration 0011: append-only SQL trial captures for review.\n"
        "-- Apply only after verified 0001, 0002, 0003, 0004, 0005, 0006, 0007, 0008, 0009 and 0010 "
        "in the dedicated enterprise_* database.\n\n"
        "BEGIN;\nSET LOCAL search_path TO public;\n\n" + ";\n\n".join(sql_trials_statements()) + ";\n\nCOMMIT;\n"
    )


def read_sql_trials_sql() -> str:
    artifact = (
        files("enterprise_platform.persistence").joinpath("migrations/0011_sql_trials.sql").read_text(encoding="utf-8")
    )
    if artifact != render_sql_trials_sql():
        raise InvalidInput("migration_artifact_mismatch")
    return artifact


def apply_sql_trials(database_url: str, expected_database: str) -> None:
    ...
    try:
        with engine.begin() as connection:
            ...
            require_sql_trial_prerequisites(inspect(connection))
            for statement in sql_trials_statements():
                connection.exec_driver_sql(statement)
    except SQLAlchemyError:
        raise PersistenceError("enterprise_sql_trials_migration_failed") from None
    finally:
        engine.dispose()
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = _Parser(description="Explicit enterprise migration 0011 after verified migrations 0001 through 0010")
    ...
    try:
        ...
        if args.print_sql:
            sys.stdout.write(read_sql_trials_sql())
            return 0
        ...
        apply_sql_trials(url, args.expected_database)
    except EnterpriseError as error:
        sys.stderr.write(error.code + "\n")
        return 1
    sys.stdout.write("Applied enterprise migration 0011.\n")
    return 0
