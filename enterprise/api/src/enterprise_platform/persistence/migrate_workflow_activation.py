"""Explicit guarded additive 0007. No native app changes or automatic migrations."""
...
def require_activation_prerequisites(inspector: Inspector, *, schema: str = "public") -> None:
    require_schema(inspector, prerequisite_metadata(), schema=schema)


def workflow_activation_statements() -> tuple[str, ...]:
    metadata = MetaData()
...
def render_workflow_activation_sql() -> str:
    return (
        ...
        + ";\n\n".join(workflow_activation_statements())
        + ";\n\nCOMMIT;\n"
    )


def read_workflow_activation_sql() -> str:
    artifact = (
        files("enterprise_platform.persistence")
        .joinpath("migrations/0007_workflow_activation.sql")
        .read_text(encoding="utf-8")
    )
    if artifact != render_workflow_activation_sql():
        raise InvalidInput("migration_artifact_mismatch")
    return artifact


def apply_workflow_activation(database_url: str, expected_database: str) -> None:
    ...
    try:
        with engine.begin() as connection:
            ...
            require_activation_prerequisites(inspect(connection))
            for statement in workflow_activation_statements():
                connection.exec_driver_sql(statement)
    except SQLAlchemyError:
        raise PersistenceError("enterprise_workflow_activation_migration_failed") from None
    finally:
        engine.dispose()
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = _Parser(
        description="Explicit enterprise migration 0007 after verified 0001, 0002, 0003, 0004, 0005 and 0006"
    )
    ...
    try:
        ...
        if args.print_sql:
            sys.stdout.write(read_workflow_activation_sql())
            return 0
        ...
        apply_workflow_activation(url, args.expected_database)
    except EnterpriseError as error:
        sys.stderr.write(error.code + "\n")
        return 1
    sys.stdout.write("Applied enterprise migration 0007.\n")
    return 0
