"""Explicit guarded additive 0004. No native app changes or automatic migrations."""
...
def prerequisite_metadata() -> MetaData:
    metadata = MetaData()
    for source in (Base.metadata, SourceBase.metadata, SetupBase.metadata):
        for table in source.sorted_tables:
            table.to_metadata(metadata)
    return metadata


def require_credential_prerequisites(inspector: Inspector, *, schema: str = "public") -> None:
    require_schema(inspector, prerequisite_metadata(), schema=schema)


def workflow_credential_statements() -> tuple[str, ...]:
    metadata = MetaData()
    for table in CredentialBase.metadata.sorted_tables:
...
def render_workflow_credentials_sql() -> str:
    return (
        ...
        + ";\n\n".join(workflow_credential_statements())
        + ";\n\nCOMMIT;\n"
    )


def read_workflow_credentials_sql() -> str:
    artifact = (
        files("enterprise_platform.persistence")
        .joinpath("migrations/0004_workflow_credentials.sql")
        .read_text(encoding="utf-8")
    )
    if artifact != render_workflow_credentials_sql():
        raise InvalidInput("migration_artifact_mismatch")
    return artifact


def apply_workflow_credentials(database_url: str, expected_database: str) -> None:
    ...
    try:
        with engine.begin() as connection:
            ...
            require_credential_prerequisites(inspect(connection))
            for statement in workflow_credential_statements():
                connection.exec_driver_sql(statement)
    except SQLAlchemyError:
        raise PersistenceError("enterprise_workflow_credential_migration_failed") from None
    finally:
        engine.dispose()
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = _Parser(description="Explicit enterprise migration 0004 after verified 0001, 0002 and 0003")
    ...
    try:
        ...
        if args.print_sql:
            sys.stdout.write(read_workflow_credentials_sql())
            return 0
        ...
        apply_workflow_credentials(url, args.expected_database)
    except EnterpriseError as error:
        sys.stderr.write(error.code + "\n")
        return 1
    sys.stdout.write("Applied enterprise migration 0004.\n")
    return 0
