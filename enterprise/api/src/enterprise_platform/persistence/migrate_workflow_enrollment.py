"""Explicit guarded additive 0006. No native app changes or automatic migrations."""
...
def require_enrollment_prerequisites(inspector: Inspector, *, schema: str = "public") -> None:
    require_schema(inspector, prerequisite_metadata(), schema=schema)


def workflow_enrollment_statements() -> tuple[str, ...]:
    metadata = MetaData()
    for table in EnrollmentBase.metadata.sorted_tables:
...
def render_workflow_enrollment_sql() -> str:
    return (
        ...
        + ";\n\n".join(workflow_enrollment_statements())
        + ";\n\nCOMMIT;\n"
    )


def read_workflow_enrollment_sql() -> str:
    artifact = (
        files("enterprise_platform.persistence")
        .joinpath("migrations/0006_workflow_enrollment.sql")
        .read_text(encoding="utf-8")
    )
    if artifact != render_workflow_enrollment_sql():
        raise InvalidInput("migration_artifact_mismatch")
    return artifact


def apply_workflow_enrollment(database_url: str, expected_database: str) -> None:
    ...
    try:
        with engine.begin() as connection:
            ...
            require_enrollment_prerequisites(inspect(connection))
            for statement in workflow_enrollment_statements():
                connection.exec_driver_sql(statement)
    except SQLAlchemyError:
        raise PersistenceError("enterprise_workflow_enrollment_migration_failed") from None
    finally:
        engine.dispose()
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = _Parser(description="Explicit enterprise migration 0006 after verified 0001, 0002, 0003, 0004 and 0005")
    ...
    try:
        ...
        if args.print_sql:
            sys.stdout.write(read_workflow_enrollment_sql())
            return 0
        ...
        apply_workflow_enrollment(url, args.expected_database)
    except EnterpriseError as error:
        sys.stderr.write(error.code + "\n")
        return 1
    sys.stdout.write("Applied enterprise migration 0006.\n")
    return 0
