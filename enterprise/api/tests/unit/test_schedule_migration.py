"""Guarded 0008 migration behavior with injected engines only; no database connections."""
...
@pytest.fixture
def migration():
    name = "enterprise_platform.persistence.migrate_schedules"
    assert importlib.util.find_spec(name) is not None, "Guarded schedule migration must exist"
...
def test_requires_exact_eleven_table_prerequisites(migration):
    from enterprise_platform.persistence.migrate_workflow_credentials import require_credential_prerequisites

    assert len(migration.prerequisite_metadata().tables) == 11
    migration.require_schedule_prerequisites(inspector(migration))
    with pytest.raises(Conflict):
        require_credential_prerequisites(inspector(migration))


def test_missing_activation_table_is_rejected(migration):
    fake = inspector(migration)
    fake.get_table_names.return_value.remove("enterprise_workflow_activations")
    with pytest.raises(Conflict):
        migration.require_schedule_prerequisites(fake)


def test_prior_activation_artifact_verified_before_engine(migration, monkeypatch):
    monkeypatch.setattr(
        migration, "read_workflow_activation_sql", Mock(side_effect=InvalidInput("prior_artifact_drift"))
    )
    engine = Mock(side_effect=AssertionError("No database"))
    monkeypatch.setattr(migration, "create_engine", engine)
    with pytest.raises(InvalidInput):
        migration.apply_schedules("postgresql+psycopg://user:secret@host/enterprise_test", "enterprise_test")
    engine.assert_not_called()


def test_activation_constraint_drift_is_rejected(migration):
    fake = inspector(migration)
    original = fake.get_unique_constraints.side_effect
    fake.get_unique_constraints.side_effect = lambda name, **kw: (
        [] if name == "enterprise_workflow_activations" else original(name, **kw)
    )
    with pytest.raises(Conflict):
        migration.require_schedule_prerequisites(fake)
...
def test_artifact_print_is_exact_additive_and_does_not_connect(migration, monkeypatch, capsys):
    from enterprise_platform.persistence.schedule_models import ScheduleBase

    before = {table.name: (len(table.constraints), table.schema) for table in ScheduleBase.metadata.sorted_tables}
    sql = migration.read_schedules_sql()
    assert sql == migration.render_schedules_sql()
    assert sql == migration.render_schedules_sql()
    assert before == {
        table.name: (len(table.constraints), table.schema) for table in ScheduleBase.metadata.sorted_tables
    }
    ...
    for name in ScheduleBase.metadata.tables:
...
def test_wrong_target_precedes_engine(migration, monkeypatch, url):
    ...
    monkeypatch.setattr(migration, "create_engine", engine)
    with pytest.raises(InvalidInput):
        migration.apply_schedules(url, "enterprise_test")
    engine.assert_not_called()
...
def test_apply_is_transactional_locked_and_guarded(migration, monkeypatch):
    ...
    assert [call.args[0] for call in connection.exec_driver_sql.call_args_list] == [
        ...
        *migration.schedules_statements(),
    ]
...
def test_cli_explicit_environment_and_success_message(migration, monkeypatch, capsys):
    apply = Mock()
    monkeypatch.setattr(migration, "apply_schedules", apply)
    monkeypatch.setenv("ENTERPRISE_SCHEDULE_DATABASE_URL", "postgresql+psycopg://user:secret@host/enterprise_test")
    assert (
        migration.main(
            ["--apply", "--url-env", "ENTERPRISE_SCHEDULE_DATABASE_URL", "--expected-database", "enterprise_test"]
        )
        == 0
    )
    apply.assert_called_once_with("postgresql+psycopg://user:secret@host/enterprise_test", "enterprise_test")
    assert capsys.readouterr().out == "Applied enterprise migration 0008.\n"


def test_guarded_migration_entrypoint_exists():
    assert importlib.util.find_spec("enterprise_platform.persistence.migrate_schedules") is not None


def test_cli_missing_environment_never_connects(migration, monkeypatch, capsys):
    monkeypatch.delenv("ENTERPRISE_SCHEDULE_DATABASE_URL", raising=False)
    engine = Mock(side_effect=AssertionError("No database"))
    monkeypatch.setattr(migration, "create_engine", engine)
    assert (
        migration.main(
...
def test_ddl_has_exact_scoped_journal_key_constraints_and_index(migration):
    sql = migration.read_schedules_sql()
    assert "PRIMARY KEY (workspace_id, schedule_id)" in sql
    assert "UNIQUE (workspace_id, binding_id)" in sql
    assert "enabled BOOLEAN NOT NULL" in sql
    assert "public_json TEXT NOT NULL" in sql
    assert "ck_schedule_revisions" in sql
    assert "ck_schedule_chronology" in sql
    assert "UNIQUE (workspace_id, device_id, scenario)" in sql
    assert "CREATE INDEX ix_schedule_due" in sql
...
def test_reapplication_is_rejected_before_ddl(migration, monkeypatch):
    ...
    connection.scalar.return_value = "enterprise_test"
    fake = inspector(migration)
    fake.get_table_names.return_value.append("enterprise_schedules")
    monkeypatch.setattr(migration, "create_engine", Mock(return_value=engine))
    monkeypatch.setattr(migration, "inspect", Mock(return_value=fake))
    with pytest.raises(Conflict):
        migration.apply_schedules("postgresql+psycopg://user:secret@host/enterprise_test", "enterprise_test")
...
def test_postgres_reflected_credential_checks_pass_eleven_table_guard(migration):
    fake = inspector(migration)
