...
def assess(service, actor=None):
    return asyncio.run(service.assess_sql_trial(actor or principal(), "dashboard-1", "draft-1", "trial-1"))


def test_assessment_uses_authorized_saved_capture_without_executing_or_publishing():
    service, parts, store, evidence = fixture()
    result = assess(service)
    assert result.trial_id == "trial-1"
    assert result.slot_id == "a"
    assert result.status == "sample_compatible"
...
def test_readonly_actor_never_loads_evidence_for_assessment():
    service, parts, store, evidence = fixture()
    with pytest.raises(AccessDenied):
        assess(service, principal().model_copy(update={"workspace_role": "normal"}))
    store.get.assert_not_called()


def test_revocation_during_assessment_prevents_returning_a_compatible_status():
    service, parts, store, evidence = fixture()
...
def test_changed_dashboard_rejects_assessment():
    service, parts, store, evidence = fixture()
