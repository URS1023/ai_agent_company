...
vi.mock('mermaid', () => ({
  default: {
    ...
    render: vi.fn().mockResolvedValue({
      svg: '<svg id="mermaid-chart">test-svg</svg>',
      diagramType: 'flowchart',
    }),
    mermaidAPI: {
      render: vi.fn().mockResolvedValue({
        svg: '<svg id="mermaid-chart">test-svg-api</svg>',
        diagramType: 'flowchart',
      }),
    },
  },
...
describe('Mermaid Flowchart Component', () => {
  ...
  describe('Interactions', () => {
    ...
    it('should keep selected look unchanged when clicking an already-selected look button', async () => {
      ...
      expect(vi.mocked(mermaid.mermaidAPI.render).mock.calls.length).toBe(
        afterFirstHandDrawnApiCalls,
      )
    })
    ...
    it('should toggle theme from light to dark and back to light', async () => {
      ...
      await waitFor(
        () => {
          expect(screen.getByRole('button')).toHaveAttribute(
            'title',
            expect.stringMatching(SWITCH_LIGHT_RE),
          )
        },
        { timeout: 3000 },
      )
      ...
      await waitFor(
        () => {
          expect(screen.getByRole('button')).toHaveAttribute(
            'title',
            expect.stringMatching(SWITCH_DARK_RE),
          )
        },
        { timeout: 3000 },
      )
    ...
    it('should keep previous svg visible while next render is loading', async () => {
      ...
      vi.mocked(mermaid.render)
        .mockResolvedValueOnce({
          svg: '<svg id="mermaid-chart">initial-svg</svg>',
          diagramType: 'flowchart',
        })
      ...
      resolveSecondRender!({
        svg: '<svg id="mermaid-chart">second-svg</svg>',
        diagramType: 'flowchart',
      })
      await waitFor(
        () => {
  ...
  describe('Error Handling', () => {
    ...
    it('should show container-not-found error when container ref remains null', async () => {
      ...
      vi.doMock('react', async () => {
        ...
        const mockedUseRef = ((initialValue: unknown) => {
          ...
          if (!patchedContainerRef && initialValue === null) pendingContainerRef = ref
    ...
    it('should cancel a pending handDrawn render on unmount', async () => {
      ...
      try {
        ...
        expect(vi.mocked(mermaidFresh.mermaidAPI.render).mock.calls.length).toBe(
          initialHandDrawnCalls,
        )
      } finally {
        vi.useRealTimers()
      }
